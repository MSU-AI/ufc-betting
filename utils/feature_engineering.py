...
def create_rolling_features(df, window_sizes=[3, 5, 10]):
    ...
    for window in window_sizes:
        ...
        df["away_winpct_last_{}".format(window)] = df.groupby(
            ["team_id_away", "season"]
        )["target"].transform(
            lambda x: x.shift(1).rolling(window=window, min_periods=1).mean()
        )
...
def create_shooting_efficiency(df):
    ...
    df["home_shooting_eff"] = (
        df["home_avg_fg_pct"] * 2 + df["home_avg_fg3_pct"] * 3 + df["home_avg_ft_pct"]
    ) / 6
    df["away_shooting_eff"] = (
        df["away_avg_fg_pct"] * 2 + df["away_avg_fg3_pct"] * 3 + df["away_avg_ft_pct"]
    ) / 6
...
def create_efficiency_metrics(df):
    ...
    df["home_ts_pct"] = df["home_avg_pts"] / (
        2 * (df["home_avg_fg_pct"] * 100 + 0.44 * df["home_avg_ft_pct"] * 100)
    )
    df["away_ts_pct"] = df["away_avg_pts"] / (
        2 * (df["away_avg_fg_pct"] * 100 + 0.44 * df["away_avg_ft_pct"] * 100)
    )
